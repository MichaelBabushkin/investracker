...
import { Upload, Clock, Landmark, Globe2, Building2, ArrowRight, DollarSign, X } from "lucide-react";
...
export default function PortfolioDashboard() {
  ...
  const [uploadMarket, setUploadMarket] = useState<Market>("israeli");
  ...
  const handleUploadComplete = (_results: UploadResult[]) => {
    ...
    fetchPendingCounts().then(() => {
      if (israeliPendingCount > 0) {
        setPendingMarket("israeli");
        setPendingOpen(true);
      }
    });
  ...
  const handleWorldUploadComplete = (_results: WorldStockUploadResult[]) => {
    setUploadOpen(false);
    fetchPendingCounts().then(() => {
      if (worldPendingCount > 0) {
        setPendingMarket("international");
        setPendingOpen(true);
      }
    });
    setRefreshTrigger((t) => t + 1);
  ...
  return (
    <div className="min-h-screen bg-surface-dark px-4 py-6 lg:px-8 lg:py-8">
      ...
      {uploadOpen && (
        ...
        >
          <div className="bg-surface-dark-secondary border border-white/10 rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto">
            {/* Modal header */}
            ...
            {/* Modal body */}
            <div className="p-6">
              {uploadMarket === "israeli" ? (
                <BrokerUploader onUploadComplete={handleUploadComplete} />
              ) : (
                <WorldStockUploader onUploadComplete={handleWorldUploadComplete} />
              )}
            </div>
          </div>
        </div>
