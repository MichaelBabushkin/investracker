...
export default function IsraeliStockHoldings({ refreshTrigger }: IsraeliStockHoldingsProps) {
  ...
  const totalCost = Array.isArray(holdings) ? holdings.reduce((sum, holding) => sum + (holding.purchase_cost || 0), 0) : 0
  ...
  const pieChartData = Array.isArray(holdings) ? holdings.map((holding, index) => ({
    name: holding.symbol,
  ...
  const COLORS = [
    ...
    '#87D068', '#FFB347', '#FFD700', '#FF6B6B', '#4ECDC4'
  ]
    ...
    <div className="space-y-6">
      ...
      {Array.isArray(holdings) && holdings.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div className={`metric-card bg-gradient-to-r ${totalReturn >= 0 ? 'from-green-500 to-green-600' : 'from-red-500 to-red-600'}`}>
            <div className="flex items-center">
              {totalReturn >= 0 ? (
          ...
          <div className={`metric-card bg-gradient-to-r ${totalReturnPercentage >= 0 ? 'from-green-500 to-green-600' : 'from-red-500 to-red-600'}`}>
            <div className="flex items-center">
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {holdings.map((holding) => {
                      const returnData = calculateReturn(holding.current_value, holding.purchase_cost)
                      return (
                        <tr key={holding.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            <div className={`font-medium ${returnData.amount >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                              {formatCurrency(returnData.amount)}
                            </div>
                            <div className={`text-xs ${returnData.percentage >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              ...
              <div className="bg-white rounded-lg shadow-md border border-gray-200 p-6">
                ...
                <div className="space-y-3 max-h-80 overflow-y-auto">
                  {pieChartData.map((item, index) => {
                    const holding = holdings.find(h => h.symbol === item.name)
                    const returnData = calculateReturn(holding?.current_value, holding?.purchase_cost)
                    return (
                      <div key={item.name} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        ...
                        <div className="text-right">
                          ...
                          <p className={`text-sm ${returnData.percentage >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                            {formatPercentage(returnData.percentage)}
                          </p>
                        </div>
                      </div>
