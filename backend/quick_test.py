#!/usr/bin/env python3
"""
Quick test to verify the improved CSV type determination
"""
...
def test_improved_processing():
    ...
    print(f"🔍 Testing improved processing: {pdf_path}")
    print("=" * 60)
    ...
    try:
        ...
        # If we found transactions now, that's good!
        if result.get('transactions_found', 0) > 0 or result.get('dividends_found', 0) > 0:
            print(f"\n🎉 SUCCESS! Now finding transactions and dividends!")
        else:
            print(f"\n❌ Still not finding transactions/dividends")
